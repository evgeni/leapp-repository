...
def pes_events_scanner(pes_json_directory, pes_json_filename):
    ...
    produce_messages(tasks, installed_pkgs)
...
def parse_modulestream(package):
    modulestream = package.get('modulestream')
    # in an older version of PES data, the field for module stream can be absent
    if not modulestream:
        return None
    return (modulestream['name'], modulestream['stream'])
...
def produce_messages(tasks, installed_pkgs):
    ...
    enabled_modules = [(m.name, m.stream) for m in enabled_modules_msg.modules]
    ...
    if to_install_pkgs or to_remove_pkgs:
        ...
        # every sublist indexed by module (name and stream) is a list of package names
        to_install_pkgs_by_module = defaultdict(list)
        for p in to_install_pkgs:
            if p[1]:
                to_install_pkgs_by_module[p[1]].append(p[0])
        to_remove_pkgs_by_module = defaultdict(list)
        for p in to_remove_pkgs:
            if p[1]:
                to_remove_pkgs_by_module[p[1]].append(p[0])
        installed_pkgs_by_module = defaultdict(list)
        for p in installed_pkgs:
            if p[1]:
                installed_pkgs_by_module[p[1]].append(p[0])

        modules_to_enable = []
        for module in to_install_pkgs_by_module:
            if module not in enabled_modules:
                modules_to_enable.append(Module(name=module[0], stream=module[1]))

        modules_to_reset = []
        #  modules where all installed packages belonging to them are going to be removed
        for module, pkgs in installed_pkgs_by_module.items():
            for p in pkgs:
                if module in enabled_modules and all(p in to_remove_pkgs_by_module[module] for p in pkgs):
