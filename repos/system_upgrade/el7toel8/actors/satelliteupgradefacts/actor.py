    ...
    def process(self):
        ...
        if local_postgresql:
            ...
            old_pgsql_data = bool(os.path.exists('/var/lib/pgsql/data/') and os.listdir('/var/lib/pgsql/data/')
                                  and os.path.exists(scl_psql_path) and os.listdir(scl_psql_path))
            scl_psql_stat = os.stat(scl_psql_path)
            ...
            if scl_psql_stat.st_dev != nonscl_psql_stat.st_dev:
                ...
                scl_du_call = run(['du', '--block-size=1', '--summarize', scl_psql_path])
            ...
            # Registers the postgresql workaround script
            self.produce(
                DNFWorkaround(
                    display_name='PostgreSQL legacy actions',
                    script_path=os.path.abspath(
                        self.get_tool_path('handle-postgresql-legacy-actions')
                    ),
                )
            )
