...
from leapp.models import InstalledRPM, SatelliteFacts, RPM, Module, RpmTransactionTasks
...
foremanproxy_RPM = fake_package('foreman-proxy')
...
def test_satellite_capsule_present(current_actor_context):
    current_actor_context.feed(InstalledRPM(items=[foremanproxy_RPM]))
    current_actor_context.run()
    message = current_actor_context.consume(SatelliteFacts)[0]
    assert message.has_foreman
